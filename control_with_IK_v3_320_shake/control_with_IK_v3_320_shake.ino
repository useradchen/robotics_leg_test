...
bool initializeDynamixels() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        dxl_wb.itemWrite(dxl_id[i], "Position_P_Gain", 800);  // 降低P增益
        dxl_wb.itemWrite(dxl_id[i], "Position_I_Gain", 0);    // 设置I增益
        dxl_wb.itemWrite(dxl_id[i], "Position_D_Gain", 0);    // 设置D增益
        dxl_wb.itemWrite(dxl_id[i], "Moving_Speed", 50);      // 降低移动速度
        dxl_wb.itemWrite(dxl_id[i], "Moving_Threshold", 10);  // 增加移动阈值
    }
...
TrajectoryCoefficients calculatePolynomialCoefficients(
    float x0, float v0, float a0,  // 初始位置、速度、加速度
    float xf, float vf, float af,  // 终止位置、速度、加速度
    float T                        // 总时间
) {
    TrajectoryCoefficients coef;
    float T2 = T * T;
    float T3 = T2 * T;
    float T4 = T3 * T;
    float T5 = T4 * T;
    ...
    coef.a1 = v0;
    coef.a2 = a0/2.0;
    coef.a3 = (20*xf - 20*x0 - (8*vf + 12*v0)*T - (3*a0 - af)*T2)/(2*T3);
    coef.a4 = (30*x0 - 30*xf + (14*vf + 16*v0)*T + (3*a0 - 2*af)*T2)/(2*T4);
    coef.a5 = (12*xf - 12*x0 - (6*vf + 6*v0)*T - (a0 - af)*T2)/(2*T5);
    
    return coef;
}

// 计算某一时刻的轨迹点
TrajectoryPoint calculatePolynomialTrajectory(
    const TrajectoryCoefficients& coef,
    float t
) {
    TrajectoryPoint point;
    float t2 = t * t;
    ...
    point.x = coef.a0 + coef.a1*t + coef.a2*t2 + coef.a3*t3 + coef.a4*t4 + coef.a5*t5;
    // 速度可以通过对位置求导获得
    float v = coef.a1 + 2*coef.a2*t + 3*coef.a3*t2 + 4*coef.a4*t3 + 5*coef.a5*t4;
    point.z = v;  // 这里用速度来控制z方向的运动

    return point;
}

// 修改步态生成函数
bool generateGaitCycle(float distance, float time) {
    // 定义关键点参数
    float x0 = 0;      // 初始位置
    float v0 = 0;      // 初始速度
    float a0 = 0;      // 初始加速度
    float xf = distance; // 目标位置
    float vf = 0;      // 终止速度
    float af = 0;      // 终止加速度

    // 计算多项式系数
    TrajectoryCoefficients swingCoef = calculatePolynomialCoefficients(
        x0, v0, a0, xf, vf, af, time
    );
    
    // 支撑腿的系数(相反方向)
    TrajectoryCoefficients stanceCoef = calculatePolynomialCoefficients(
        -x0, -v0, -a0, -xf, -vf, -af, time
    );
