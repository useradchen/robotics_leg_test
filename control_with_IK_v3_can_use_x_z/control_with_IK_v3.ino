...
bool moveToInitialPosition() {
    ...
    int32_t initialPositions[4] = {
        convertRadianToPosition(-(initialAngles.theta_hp+M_PI)),    // 左腿 Hip pitch
        convertRadianToPosition(initialAngles.theta_ap-M_PI),       // 左腿 Ankle pitch
        convertRadianToPosition(-(initialAngles.theta_hp+M_PI)),    // 右腿 Hip pitch
        convertRadianToPosition(initialAngles.theta_ap-M_PI)        // 右腿 Ankle pitch
    };
...
bool moveWithAdaptiveSpeed(int32_t targetPositions[4], int32_t currentPositions[4]) {
    ...
    // 如果位置差異太小，直接返回成功
    if (maxDiff < 5) {
        Serial.println("位置差異太小，無需移動");
        return true;
    }
    
    // 根據最大差異自動計算分段數
    // 調整分段計算方式
    int totalSteps = maxDiff / 5;  // 每5個位置值為一步
    if (totalSteps < 50) totalSteps = 50;    // 最少50步
    if (totalSteps > 200) totalSteps = 200;  // 最多200步
    
    // 計算基礎延遲時間
    int baseDelay = 10;  // 固定基礎延遲
...
bool moveToCoordinate(float x, float y, float z) {
    ...
    int32_t targetPositions[4] = {
        convertRadianToPosition(-(leftLegAngles.theta_hp+M_PI)),    // 左腿 Hip pitch
        convertRadianToPosition(leftLegAngles.theta_ap-M_PI),       // 左腿 Ankle pitch
        convertRadianToPosition(-(rightLegAngles.theta_hp+M_PI)),   // 右腿 Hip pitch
        convertRadianToPosition(rightLegAngles.theta_ap-M_PI)       // 右腿 Ankle pitch
    };
    
    // 顯示角度資訊
    Serial.println("\n=== 角度資訊（弧度） ===");
    Serial.print("左腿 Hip: "); Serial.print(-(leftLegAngles.theta_hp+M_PI));
    Serial.print(", Ankle: "); Serial.println(leftLegAngles.theta_ap-M_PI);
    Serial.print("右腿 Hip: "); Serial.print(-(rightLegAngles.theta_hp+M_PI));
    Serial.print(", Ankle: "); Serial.println(rightLegAngles.theta_ap-M_PI);
