...
float calculateRequiredSquatHeight(float stepLength) {
    const float MAX_HEIGHT_INCREMENT = 20.0;  // 最大高度增量
    const float HEIGHT_SCALE_FACTOR = 0.5;    // 控制高度變化的比例因子
    
    float squatIncrement = stepLength * HEIGHT_SCALE_FACTOR;
    Serial.println(" -------- send -------- ");
    Serial.println(squatIncrement);
    return fmin(squatIncrement, MAX_HEIGHT_INCREMENT);
}
...
float executeWalkingCycle(float step_length) {
  ...
  ***/
    ...
    if (step_length >= 20){
      requiredSquatHeight = currentSquatHeight + (step_length/2);
    }
    else{
      requiredSquatHeight = step_length;
    }

    // delay(1000); // 避免動作切換過快 (by just test)
    // 左腿動(0,0,z)抬腳
    ...
    // just test z
    // delay(500);
    if(!moveToCoordinate(step_length, 0, 0, true)) {
        Serial.println("Failed to lift left leg!");
        return 0.0; // false
    }
...
// =========================================================================================
    ...
    // just test z
    delay(100);
    if(!moveToCoordinate(step_length, 0, 0, false)) {
        Serial.println("Failed to lift left leg!");
        return 0.0; // false
    }
    delay(100);

    if(!moveToCoordinate(0, 0, 0, false)) {
        Serial.println("Failed to lift left leg!");
        return 0.0; // false
    }
